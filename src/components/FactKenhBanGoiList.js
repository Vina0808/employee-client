...
function FactKenhBanGoiList() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setDropdowns({
        ...
        hinhthuctb: json.filters?.hinhthuctb || [],
  ...
  const mapToOptions = (arr) =>
    Array.isArray(arr) ? arr.map((item) => ({ value: item, label: item })) : [];
  ...
  return (
    <div style={{ fontFamily: 'Arial, sans-serif', padding: '10px' }}>
      ...
      <div style={{ marginBottom: '15px', padding: '10px', backgroundColor: '#E3F2FD', borderRadius: '5px', display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center' }}>
        ...
        <CustomSelect value={filters.phongBanHRM} onChange={(val) => handleCustomSelectChange('phongBanHRM', val)} options={mapToOptions(dropdowns.phongBanHRM)} placeholder="-- Phòng ban HRM --" />
        ...
        <CustomSelect value={filters.congcubangoi} onChange={(val) => handleCustomSelectChange('congcubangoi', val)} options={mapToOptions(dropdowns.congcubangoi)} placeholder="-- Công cụ bán gói --" />
        ...
        <CustomSelect value={filters.hinhthuctb} onChange={(val) => handleCustomSelectChange('hinhthuctb', val)} options={mapToOptions(dropdowns.hinhthuctb)} placeholder="-- Hình thức TB --" />
        ...
        <div>
          ...
          <table style={{ width: '100%', fontSize: '14px', borderCollapse: 'collapse', border: '1px solid #ccc' }}>
            <thead style={{ backgroundColor: '#f2f2f2' }}>
              <tr>
                {['STT', 'HRM BH', 'Nhân viên', 'Phòng ban HRM', 'Số TB', 'User BH', 'Ngày BH', 'Gói cước', 'Chu kỳ gói', 'Giá gói', 'Loại TB Tháng', 'Hình thức TB', 'Công cụ bán gói', 'Thuê bao HVC', 'Loại giao dịch', 'Dịch vụ viễn thông', 'Tháng DL'].map((header, idx) => (
