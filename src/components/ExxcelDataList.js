...
const ExcelDataList = () => {
  ...
  const handleAcceptClick = async () => {
    try {
      const now = new Date(); // ✅ lấy thời gian máy tính hiện tại
      const Ngay_dieu_chinh = now.toISOString(); // format chuẩn ISO gửi lên backend
              ...
              {Object.entries(item)
                ...
                .map(([key, value]) => (
                  <td key={key} style={{ padding: '8px', border: '1px solid #ccc' }}>
                    {(key === 'Thoi_gian_giao_dich' || key === 'ngay_cn' || key === 'Ngay_dieu_chinh') && value
